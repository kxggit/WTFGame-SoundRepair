...
(function (window, Object, undefined) {
	...
	Object.defineProperties(RigidBody.prototype, {
		...
		angularAcceleration: {
			...
			set: function (v) {
				this.phaser.body.angularAcceleration = v;
		...
		angularVelocity: {
			...
			set: function (v) {
				this.phaser.body.angularVelocity = v;
				this.gameObject._isTransformDirty = true;
			}
		},
		...
		angularDrag: {
			...
			set: function (v) {
				this.phaser.body.angularDrag = v;
				this.gameObject._isTransformDirty = true;
			}
		},
	...
	DialogBubble.prototype.show = function (dialog, uuid) {
		...
		if (!SoundManager.instance.mute) {
			...
			audio.src = 'https://dict.youdao.com/dictvoice?audio=' + encodeURIComponent(dialog) + '&le=zh';
	...
	*/
	...
	LoginManager.prototype.onClickStart = function () {
		...
		if (GameManager.instance.getLenth(username.trim()) == 0) {
			this.showTip('请输入论坛邮箱！');
			return;
		}
		var emailTest = /^(\w-*\.*)+@(\w-?)+(\.\w{2,})+$/, result = emailTest.test(username);
		if (!result) {
			this.showTip("邮箱格式错误！(AAAA@BB.CC)");
			return
		}
		var password = this.inpPassword.text.trim();
		if (GameManager.instance.getLenth(password.trim()) == 0) {
			this.showTip("请输入论坛密码！");
			return
		}
		if (GameManager.instance.getLenth(username.trim()) > 80) {
			this.showTip('用户名太长！');
			return;
		}
		if (this.dropdown.value < 0 || this.dropdown.value > 2) {
		...
		this.game.storage.set('password', password);
	...
	LoginManager.prototype.onOpen = function () {
		...
		var storedPassword = this.game.storage.get('password');
		if (storedName != null && storedName != '') {
			this.inpUsername.text = storedName;
			this.inpUsername.placeholder.text = "";
		}
		if (storedPassword != null && storedPassword != '') {
			this.inpPassword.text = storedPassword;
			this.inpPassword.placeholder.text = "";
		}
	};
